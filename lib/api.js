...
export const createZeroTrustListsOneByOne = async (items) => {
  ...
  const pauseExecution = () => new Promise(resolve => setTimeout(resolve, 5000));
  for (let i = 0; i < items.length; i += LIST_ITEM_SIZE) {
    if (iterationCount >= 20) {
      console.log('Pausing execution for 5 seconds because limited API...');
...
export const deleteZeroTrustListsOneByOne = async (lists) => {
  ...
  const pauseExecution = () => new Promise(resolve => setTimeout(resolve, 30000));
