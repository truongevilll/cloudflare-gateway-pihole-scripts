...
export const createZeroTrustListsOneByOne = async (items) => {
  ...
  const pauseExecution = () => new Promise(resolve => setTimeout(resolve, 70000));
  for (let i = 0; i < items.length; i += LIST_ITEM_SIZE) {
    if (iterationCount >= 45) {
      console.log('Pausing execution for 70 seconds because limited API...');
...
export const deleteZeroTrustListsOneByOne = async (lists) => {
  ...
  const pauseExecution = () => new Promise(resolve => setTimeout(resolve, 60000));

  for (const { id, name } of lists) {
    if (iterationCount >= 50) {
      console.log('Pausing execution for 60 seconds because limited API...');
      await pauseExecution();
      iterationCount = 0;
    }
    try {
  ...
  console.log('Waiting for 60 seconds before next steps because limited API...');
  await pauseExecution();
};
